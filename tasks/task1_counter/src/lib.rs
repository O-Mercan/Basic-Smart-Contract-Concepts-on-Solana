...
mod test {
    ...
    fn test_counter() {
        ...
        let increment_instruction_data: Vec<u8> = [0].iter().chain(&increment_value.to_le_bytes()).cloned().collect();
        let decrement_value: u32 = 3;
        let decrement_instruction_data: Vec<u8> = [1].iter().chain(&decrement_value.to_le_bytes()).cloned().collect();
